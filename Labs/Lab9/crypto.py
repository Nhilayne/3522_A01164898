...
from decoding_handler import DecodingHandler
...
class Crypto:
    ...
    def execute_request(self, request: Request):
        if request.encryption_state == CryptoMode.EN:
            self.encryption_start_handler.handle(request)
        else:
            self.decryption_start_handler.handle(request)


def main(request: Request):
    coder = Crypto()
    coder.execute_request(request)
