...
class Troublemaker(User):
    """
    Stores data related to a troublemaker usertype
    """

    def __init__(self, name, age, bank_info, budget_list):
        """
        Initialize Troublemaker object
    ...
    def warnings(self, current_budget):
        ...
        :return: flag thrown as boolean
        """
        # warn they're over 75% of their budget each time
        if (float(current_budget.get_budget_spent()) / float(current_budget.get_budget_target())) > 0.75:
            print("WARNING: You have exceeded 75% of this budget")
            return True
        return False
    ...
    def lockouts(self, current_budget):
        ...
        if (float(current_budget.get_budget_spent()) / float(current_budget.get_budget_target())) > 1.2:
